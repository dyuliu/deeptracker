  ...
  class Controller {
    ...
    constructor(
          ...
          if (!_.isEmpty(qArray)) {
            $q.all(qArray).then(kernelData => {
              ...
              _.each(kernelData, (v, k) => {
                ...
                $scope.optionsTopK[k] = self._setOptions('heatline');
      ...
      function openPixelChart(name) {
        ...
        DataManager.fetchKernel({ db, type, layer: [lid], parser }, false)
          ...
          .then(data => {
            $('#layer-data-loading').addClass('invisible');
            $scope.optionsDetail[name] = self._setOptions('pixelChartWithLine');
      ...
      function act(conf) {
        ...
        if (conf.chartType === 'lineChart') {
          ...
          for (let d of tree) {
            ...
            $scope.options[d.name] = self._setOptions('sparklinePlus');
            $scope.size[d.name] = 0;
            if (!d.nodes) {
            ...
            if (d.nodes) {
              ...
              for (let dn of d.nodes) {
                ...
                $scope.options[dn.name] = self._setOptions('sparklinePlus');
                ...
                if (dn.nodes) {
                  ...
                  for (let dnn of dn.nodes) {
                    ...
                    $scope.options[dnn.name] = self._setOptions('sparklinePlus');
          ...
          $q.all([
            ...
            let tmpStat = self._processData('stat', layers, data[0]),
              tmpHlStat = self._processData('hl_stat', hlLayers, data[1]);
          ...
          for (let d of tree) {
            $scope.opened[d.name] = false;
            $scope.options[d.name] = self._setOptions('boxPlotChart');
            $scope.showTypes[d.name] = 'nvd3';
            layerTree[d.name] = d;
            if (d.nodes) {
              hlLayers.push(d.name);
              for (let dn of d.nodes) {
                dn.parent = d;
                $scope.opened[dn.name] = false;
                $scope.options[dn.name] = self._setOptions('boxPlotChart');
                $scope.showTypes[dn.name] = 'nvd3';
                layerTree[dn.name] = dn;
                if (dn.nodes) {
                  hlLayers.push(dn.name);
                  for (let dnn of dn.nodes) {
                    dnn.parent = dn;
                    $scope.opened[dnn.name] = false;
                    $scope.options[dnn.name] = self._setOptions('boxPlotChart');
                    $scope.showTypes[dnn.name] = 'nvd3';
                    layerTree[dnn.name] = dnn;
                  };
                }
          ...
          $q.all(queryQueue).then(data => {
            ...
            let tmpStat = self._processData('boxPlot', layers, data.slice(0, 5)),
              tmpHlStat = self._processData('hl_boxPlot', hlLayers, data.slice(5, 10));
            let [min, max] = [Math.min(tmpStat[1], tmpHlStat[1]), Math.max(tmpStat[2], tmpHlStat[2])];
            _.each($scope.options, (d: any) => {
              d.chart.width = iterSet.size;
              if (conf.sameScale) { d.chart.yDomain = [min, max]; }
          ...
          for (let d of tree) {
            $scope.opened[d.name] = false;
            $scope.optionsCR[d.name] = self._setOptions('crChart');
            ...
            if (d.nodes) {
              ...
              for (let dn of d.nodes) {
                dn.parent = d;
                $scope.opened[dn.name] = false;
                $scope.optionsCR[dn.name] = self._setOptions('crChart');
                ...
                if (dn.nodes) {
                  ...
                  for (let dnn of dn.nodes) {
                    dnn.parent = dn;
                    $scope.opened[dnn.name] = false;
                    $scope.optionsCR[dnn.name] = self._setOptions('crChart');
          ...
          $q.all([
            ...
            _.merge(
              $scope.dataCR,
              self._processData('seq', layers, data[0]),
              self._processData('hl_seq', hlLayers, data[1])
            );
          ...
          for (let d of tree) {
            $scope.opened[d.name] = false;
            $scope.optionsHGraph[d.name] = self._setOptions('horizonGraph');
            ...
            if (d.nodes) {
              ...
              for (let dn of d.nodes) {
                dn.parent = d;
                $scope.opened[dn.name] = false;
                $scope.optionsHGraph[dn.name] = self._setOptions('horizonGraph');
                ...
                if (dn.nodes) {
                  ...
                  for (let dnn of dn.nodes) {
                    dnn.parent = dn;
                    $scope.opened[dnn.name] = false;
                    $scope.optionsHGraph[dnn.name] = self._setOptions('horizonGraph');
          ...
          $q.all([
            ...
            data[0] = _.filter(data[0], (d: any) => iterSet.has(d.iter));
            data[1] = _.filter(data[1], (d: any) => iterSet.has(d.iter));
            let tmpStat = self._processData('stat_hgraph', layers, data[0]),
              tmpHlStat = self._processData('hl_stat_hgraph', hlLayers, data[1]);
            let [min, max] = [Math.min(tmpStat[1], tmpHlStat[1]), Math.max(tmpStat[2], tmpHlStat[2])];
        ...
        $scope.tree = self._treeDataConstruction($scope.dataTree);
    ...
    private _init() {
      ...
      $('.vl-div-global').on('nvd3-mouse-hover', function (evt, msg) {
        self.Pip.emitTimeMouseMove({ point: [msg, 0], x: 0, y: 0, k: 1 });
      });
    ...
    private _setOptions(type) {
      ...
      switch (type) {
        ...
        case 'heatline':
          options = {
            width: self.Global.getData('iter').num,
            height: 12,
            cellWidth: 1,
