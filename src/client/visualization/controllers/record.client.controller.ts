  ...
  class Controller {
    ...
    constructor(
      ...
      function act(data, conf) {
        ...
        if ($scope.show.lr) {
          $scope.data.lr = self._process('sparklinePlus', data.lr);
          // $scope.options.lr = self._setOption('sparklinePlus', data.lr.length, color);
        ...
        if ($scope.show.error) {
          if (conf.testError && conf.trainError) {
            $scope.data.error = self._process('lineChart', data.testError, data.trainError);
            $scope.options.error = self._setOption('lineChart', data.testError.length, color);
          } else {
            let myColor = color;
            if (conf.trainError) { myColor = color.slice(1); }
            let d = conf.testError ? data.testError : data.trainError;
            $scope.data.error = self._process('sparklinePlus', d);
            $scope.options.error = self._setOption('sparklinePlus', d.length, myColor);
          }
        }
        if ($scope.show.loss) {
          if (conf.testLoss && conf.trainLoss) {
            $scope.data.loss = self._process('lineChart', data.testLoss, data.trainLoss);
            $scope.options.loss = self._setOption('lineChart', data.testLoss.length, color);
          } else {
            let myColor = color;
            if (conf.trainLoss) { myColor = color.slice(1); }
            let d = conf.testLoss ? data.testLoss : data.trainLoss;
            $scope.data.loss = self._process('sparklinePlus', d);
            $scope.options.loss = self._setOption('sparklinePlus', d.length, myColor);
          }
        }
      };
        ...
        case 'sparklinePlus':
          options = {
            chart: {
              type: 'sparklinePlus',
              height: 50,
              width: width ? width : self.Global.getData('iter').num,
              margin: {
                left: 0,
                right: 0,
                top: 1,
              ...
              interactiveLayer: {
                dispatch: {
                  elementMousemove: function (msg) {
                    self.Pip.emitTimeMouseMove({ point: [msg.mouseX, msg.mouseY], x: 0, y: 0, k: 1 });
                  }
                }
              },
              noData: ' ',
