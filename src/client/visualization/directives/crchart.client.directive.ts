namespace application {
  ...
  class Painter {
    ...
    public render(data: IDTypeCrChart, Pip: IPipService, scope, Global: IGlobalService) {
      ...
      self._paintContext(self.fakeCanvas.node().getContext('2d'), data);
      ...
      scope.$on('zoom', (evt, msg: any) => {
        ...
        if (scope.data && scope.options.name === msg.name) {
          ...
          console.log('redraw');
          let [dw, dh] = [data.iter.length, self.options.height];
          self.container
            .style('width', (dw + 15) + 'px')
    ...
    private _paintContext(ctx: CanvasRenderingContext2D, data) {
      let self = this;
      let color = self._buildColorMap(data.value[0].length + 1);
      let oh, nowh, lasth;
      let h = (mul: number) => self.height * mul;
      _.each(data.value, (d, idx) => {
        [oh, nowh, lasth] = [0, 0, self.height];
        _.each(d, (o, k) => {
          nowh = h(o);
