namespace application {
  ...
  class Painter {
    ...
    public render(Pip: IPipService, scope: any) {
      ...
      let data = self.nodesDataConstruction(self.data);

      // move g to the center
      let rects = self.svg.append('g')
        .attr('class', 'node-group')
        .attr('transform', 'translate(' + (self.options.width / 3) + ', 0)');
      ...
      for (let i = 1; i < data.length; i += 1) {
        if (data[i].type === 'conv-left') {
          ...
          fy.push(fy[i - 1] + self.options.node.height + self.options.space);
        }
      }
      ...
      rects.selectAll('.layer')
        ...
        .attr('x', d => {
          ...
          if (d.type === 'conv-left') { return -15 - self.options.node.width; }
          return -self.options.node.width / 2;
        })
        .attr('y', (d, i) => fy[i])
        .attr('width', self.options.node.width)
        .attr('height', self.options.node.height)
        .on('click', function (d) {
          Pip.emitFlip(d.name);
        })
      ...
      links.selectAll('.layer')
        ...
        .attr('d', (d, i) => {
          ...
          if (d.type === 'other' && d.edgeType) {
            ...
            let st;
            let ti = +i + 1;
            st = [15 + self.options.node.width / 2, fy[ti]];
            pd.push(st);

            pd.push([0, fy[i] + self.options.node.height]);

            if (d.edgeType === 1) {
              pd.push([-15 - self.options.node.width / 2, fy[ti]]);
            } else {
              pd.push([-15, fy[ti]]);
              pd.push([-15, fy[i + 3] + self.options.node.height]);
            }

            let ed;
            ti = +i + 4;
            if (d.edgeType === 1) {
              ed = [-15 - self.options.node.width / 2, fy[ti]];
            } else {
              ed = [0, fy[ti]];
            }
            pd.push(ed);
            ...
            if (d.type === 'conv-right') {
              st = [15 + self.options.node.width / 2, fy[i] + self.options.node.height];
            } else if (d.type === 'conv-left') {
              st = [-15 - self.options.node.width / 2, fy[i] + self.options.node.height];
            } else {
              st = [0, fy[i] + self.options.node.height];
            }
            pd.push(st);
      ...
      let barData = self.barDataConstruction(self.data, location);
      let barEdgeColor = {};
      _.each(barData, d => {
        barEdgeColor[d.name] = { color: '#2089ed' };
      });
      let stretchData = self.stretchData(self.data);
      let levelLinkData = self.levelLinkDataConstruction(self.data);
      let bars = self.svg.append('g')
        .attr('transform', 'translate(' + (self.options.width / 3) + ', 0)');
      ...
      bars.selectAll('.bar')
        ...
        .attr('x', d => {
          let t;
          if (d.level === 'level1') { t = 15 + self.options.node.width + 20; }
          if (d.level === 'level2') { t = 15 + self.options.node.width + 12; }
          if (d.level === 'level3' && !d.left) {
            t = 15 + self.options.node.width + 4;
        ...
        .attr('fill', '#3093f2')
        .style('opacity', d => {
          if (stretchData[d.name].parent && !self.options.opened[stretchData[d.name].parent.name]) {
            return 0.2;
          }
          if (!self.options.opened[d.name]) { return 1; }
          return 0.2;
        })
        .on('click', clickHandler);

      let barEdges = self.svg.append('g')
        .attr('transform', 'translate(' + (self.options.width / 3) + ', 0)');

      let barEdgeNodes = self.svg.append('g')
        .attr('transform', 'translate(' + (self.options.width / 3) + ', 0)');

      let levelNodeEdges = self.svg.append('g')
        .attr('transform', 'translate(' + (self.options.width / 3) + ', 0)');
      ...
      setTimeout(function () {
        barEdges.selectAll('.bar-edge')
          ...
          .attr('d', d => {
            ...
            let e = $('#' + d.name);
            if (e.position()) {
              ed = [2 * self.options.width / 3, $('#' + d.name).position().top - 19];
            } else {
              ed = st;
            }
          ...
          .style('opacity', d => {
            ...
            if (!self.options.opened[d.name]) { return 0.6; }
            return 0;
          });
        ...
        //   .attr('cx', 2 * self.options.width / 3 - 12)
        //   .attr('cy', (d: any) => {
        //     let ed;
        //     let e = $('#' + d.name);
        //     if (e.position()) {
        ...
        //   .attr('x1', 2 * self.options.width / 3 - 12)
        //   .attr('y1', (d: any) => {
        //     return $('.bar-edge-node-' + d[0].name).attr('cy');
        //   })
        //   .attr('x2', 2 * self.options.width / 3 - 12)
        //   .attr('y2', (d: any) => {
      ...
      function clickHandler(d) {
        let open = !self.options.opened[d.name];
        if (open && stretchData[d.name].nodes) {
          let p = stretchData[d.name].parent;
          if (!(p && !self.options.opened[p.name])) {
            $(this).css('opacity', 0.1);
            self.options.opened[d.name] = true;
          ...
          $(this).css('opacity', 1);
          self.options.opened[d.name] = false;
          for (let o of stretchData[d.name].nodes) {
            $('.bar-' + o.name).css('opacity', 0.1);
            self.options.opened[o.name] = false;
            if (o.nodes) {
              for (let oo of o.nodes) {
                $('.bar-' + oo.name).css('opacity', 0.1);
                self.options.opened[oo.name] = false;
              }
            }
          }
        }
        ...
        setTimeout(function () {
          barEdges.selectAll('.bar-edge')
            .attr('d', (o: any) => {
              ...
              let t;
              if (o.level === 'level1') { t = 15 + self.options.node.width + 20; }
              if (o.level === 'level2') { t = 15 + self.options.node.width + 12; }
              ...
              let e = $('#' + o.name);
              if (e.position()) {
                ed = [2 * self.options.width / 3, $('#' + o.name).position().top - 19];
              } else {
                ed = st;
              }

              if (o.level === 'level3' && o.left) {
                let tm = 15 + self.options.node.width + 16;
                let tmpArray = [];
                st[1] += 3;
                tmpArray.push(st);
            ...
            .style('stroke-width', 2)
            .style('opacity', (o: any) => {
              if (stretchData[o.name].parent && !self.options.opened[stretchData[o.name].parent.name]) {
                return 0;
              }
              if (!self.options.opened[o.name]) { return 0.6; }
              return 0;
            });
    ...
    private barDataConstruction(data, location) {
      ...
      for (let d of data) {
        let dst, ded;
        if (!d.nodes) {
          dst = location[d.name]; ded = dst + self.options.node.height;
    ...
    private nodesDataConstruction(data) {
      ...
      _.each(data, (d, di) => {
        if (d.name === 'conv1') {
          let info = self.options.layers[d.name];
          ...
          _.each(d.nodes, (dd, ddi) => {
            ...
            _.each(tmp, o => {
              let info = self.options.layers[o.name];
              nodes.push({ name: o.name, type: 'conv-right', info });
            });
            if (dd.nodes.length === 4) {
              let info = self.options.layers[dd.nodes[3].name];
