 ...
 */
logger.getLogOptions = function getLogOptions(configOptions): any {

  let _config: mean.IAppConfig = _.cloneDeep(config);
  if (configOptions) {
...
logger.getLogFormat = function getLogFormat() {
  let format = config.log.format ? config.log.format : 'combined';
