...
function getClsInfo(options: IOption, res: Response) {
  console.log(chalk.green('normal fetching ' + options.db + '_ClsInfo'));
  let col: IModel = getLayerModel(options.db + '_ClsInfo');
  let cond: any = {};
  let proj: any = { _id: 0 };
  col.find(cond, proj)
    .lean()
...
function getClsModel(collectionName: string): IModel {
  if (modelNames().indexOf(collectionName) === -1) {
    model(collectionName, clsInfoSchema, collectionName);
  }
  return model(collectionName);
};
